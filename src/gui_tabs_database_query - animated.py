...
from test_visuals import FluidCircles
...
class DatabaseQueryTab(QWidget):
    ...
    def setup_response_visualization(self):
        self.response_viz = FluidCircles(self.response_widget)
        self.response_viz.setGeometry(self.response_widget.rect())
        self.response_viz.show()
        self.response_viz.timer.start(16)

    def clear_response_visualization(self):
        if hasattr(self, 'response_viz'):
            self.response_viz.timer.stop()
            self.response_viz.hide()
            self.response_viz.deleteLater()
            delattr(self, 'response_viz')
    ...
    def on_submit_button_clicked(self):
        ...
        self.setup_response_visualization()
    ...
    def display_chunks(self, chunks):
        self.clear_response_visualization()
    ...
    def update_response_lm_studio(self, response_chunk):
        if not self.cumulative_response:
            self.clear_response_visualization()
    ...
    def update_response_local_model(self, response):
        if not self.response_widget.toPlainText():
            self.clear_response_visualization()
